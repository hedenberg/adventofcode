...
namespace Bohl.AdventOfCode.Day09;
...
public class Rope : IParsable<Rope>
{
    ...
    public void Move(bool print = true)
    {
        ...
        foreach (var (vx, vy) in Head.Moves)
        {
            ...
            {
                Tail.X += tailVx;
                if (Math.Abs(diffY) > 0) Tail.Y += tailVy;
                Tail.Locations.Add((Tail.X, Tail.Y));
            }
            else if (Math.Abs(diffY) > 1)
            {
                Tail.Y += tailVy;
                if (Math.Abs(diffX) > 0) Tail.X += tailVx;
                Tail.Locations.Add((Tail.X, Tail.Y));
            }
    ...
    public void TailPathToMap()
    {
        ...
        for (var x = 0; x < Width; x++)
        {
            if (Tail.Locations.Any(l => l.Item1 == x && l.Item2 == y)) Map[y, x] = '#';
            if (x == X && y == Y)
                Map[y, x] = 's';
        }
    }
    ...
    public void PrintState()
    {
        for (var y = Height - 1; y >= 0; y--)
        {
            for (var x = 0; x < Width; x++) Trace.Write(Map[y, x]);
            Trace.Write("\n");
        }
